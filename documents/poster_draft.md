...
反実仮想シミュレーションによって、「セオリー補正は危険な局面に対しては機能するが、状況を選ばず適用すると逆効果になりうる」という、当初の想定より一段解像度の高い知見も得られた。ただしこれは**厳密な反実仮想ではなく、擬似的な近似**である。攻撃側の行動は実観測のまま固定しており、実際には守備配置が変われば攻撃側も判断を変えた可能性がある——「攻撃側は守備側の変化に反応しない」という単純化の上に成り立っている点は、結果を読む上で明記しておく必要がある。

**今後の展望**
- コンパクトネスの目標値を試合状況（スコア差・残り時間・攻撃の幅等）に応じて動的に変える、状況依存の理論条件付け（今回、画一適用の限界が判明したことによる直接の課題）
- コンペで取得予定の大規模データ（約64試合）による再検証：検出力の向上、同じ試合・チームのデータに結果が偏るリスクの軽減
- より長期的には、攻守が相互に反応し合うマルチエージェントモデルへの拡張（反実仮想シミュレーションの「攻撃側固定」という単純化を解消する、直接の発展先）
...
- ランキング転換が①〜③のどれに効くかの内訳：③構造的バイアス（ほぼ一定オフセット）には、順位が定数加算の影響を受けないため直接効く。②誤差増幅は、ランキングというよりセットで決めた「超過逸脱度をやめてdist_B単体を使う」という設計変更の方で解決している。①疑似反復は解消されておらず、正確には「精密な推定」という疑似反復が壊すタイプの主張自体をしなくなった、という整理。上位K件が複数試合・チームにまたがっているかの軽い診断で補っている
- 「理論違反」と「単に守りにくい局面」を区別できるメカニズム：modelBは固定の理想形ではなく、観測窓（奪取直後1秒間）の攻撃側の動き・守備側の初期配置を入力として予測するため、「その局面がどれだけ厳しいか」を織り込んだ上で理論的な軌道を生成する。実際の守備とmodelBは同じ局面の厳しさを前提に比較されるため、残る乖離（dist_B）は局面の難しさそのものではなく、理論からのズレに近いものになる
- 参考にした先行研究：ハード/ソフト制約の分離は物理学のPhysics-Informed Neural Network（PINN、物理法則を損失関数に組み込む機構）、精密な推定を諦めランキングで評価する発想はマーケティングのルックアライク・オーディエンス評価を参考にしている。発表本文ではどちらの名称も出さない
