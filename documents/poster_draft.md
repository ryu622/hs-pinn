...
**反実仮想シミュレーション**：実際に成功したカウンター局面（失点相当の代理指標、n=57）について、守備側だけをPINNゴーストに差し替え、攻撃側は実観測のまま固定した上で、ピッチコントロール（守備配置に反応する危険度指標）を再計算した。**61.4%の事例でゴーストに差し替えた方が危険度が下がり、平均でも低下する方向**（delta平均-0.166）となった。
