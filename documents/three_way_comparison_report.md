...
**注記（誤読防止）**：選定した3件は偶然すべて`label=0`（失敗）だった。dist_B単体ランキング上位ほど成功率が高い傾向はLift/Precision@K評価（11.9節項目1）で確認済みだが、top10〜50件でも成功率は20〜22%程度（全体基準11.4%の約2倍、AUC=0.610）に留まる弱い確率的傾向であり、上位数件が偶然すべて失敗になることは統計的に珍しくない（top8件が全て失敗になる確率はtop50の成功率を基準にすると約14%）。本ケーススタディの目的は成功/失敗の予測精度ではなく①②③の生成物としての質の比較であり、この点は結論に影響しない。
